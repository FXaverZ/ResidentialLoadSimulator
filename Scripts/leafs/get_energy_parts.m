...
for i=1:size(households,1)
	...
	Powers_InFl.(households{i,1}) = zeros(num_timepoints,3*households{i,3}*Model.Number_Runs);	
	Powers_HePs.(households{i,1}) = zeros(num_timepoints,3*households{i,3}*Model.Number_Runs);	
end
				...
				for m=1:size(pow,1)
					for n=0:2
						...
						if idx_devs_flex(m)
							...
							powpif = powp + Powers_InFl.(act_hh_typ)((j-1)*num_tp_p_day+1:j*num_tp_p_day,...
								(i-1)*(act_hh_num*3)+(l-1)*3+1+n);
							Powers_InFl.(act_hh_typ)((j-1)*num_tp_p_day+1:j*num_tp_p_day,...
								(i-1)*(act_hh_num*3)+(l-1)*3+1+n) = powpif;
						end
						if idx_devs_energy(m)
							powphp = powp + Powers_HePs.(act_hh_typ)((j-1)*num_tp_p_day+1:j*num_tp_p_day,...
								(i-1)*(act_hh_num*3)+(l-1)*3+1+n);
							Powers_HePs.(act_hh_typ)((j-1)*num_tp_p_day+1:j*num_tp_p_day,...
...
for k=1:size(households,1)
	act_hh_typ = households{k,1};
	Power_HePs = Powers_HePs.(act_hh_typ); %#ok<NASGU>
	Powers_HePs = rmfield(Powers_HePs, act_hh_typ);
	save([source_path,filesep,'Summary',sep,datestr(simtime,'yyyy-mm-dd_HH.MM.SS'),sep,...
		'Powers_Year_HPs',sep,act_hh_typ,'.mat'],'Power_HePs','Timebase_Output','-v7.3');
	clear Power_HePs;
	Power_Flex = Powers_Flex.(act_hh_typ); %#ok<NASGU>
	Powers_Flex = rmfield(Powers_Flex, act_hh_typ);
	Power_InFl = Powers_InFl.(act_hh_typ); %#ok<NASGU>
	Powers_InFl = rmfield(Powers_InFl, act_hh_typ);
	save([source_path,filesep,'Summary',sep,datestr(simtime,'yyyy-mm-dd_HH.MM.SS'),sep,...
		'Powers_Year_FlexSep',sep,act_hh_typ,'.mat'],'Power_Flex','Power_InFl','Timebase_Output','-v7.3');
	clear Power_Flex Power_InFl;
end
fprintf('\t--> erledigt!\n');
diary('off');
