...
for a = 1:numel(xls_lpt.loadprofiles_typs)
	...
	for b=1:size(runtimes,1)
		if b == 1
			ontime = flexload_runtimelist2ontime(runtimes(b,3:end),timepoints);
		else
			ontime = ontime + flexload_runtimelist2ontime(runtimes(b,3:end),timepoints);
		end
	end
	ontime(ontime > 1) = 1;
